...
async def hex(interaction: discord.Interaction, hex_color: str):
    try:
        ...
        await interaction.response.send_message(f"Set color to RGB({red}, {green}, {blue}) from HEX({hex_color})")
    except ValueError:
        await interaction.response.send_message("Invalid hex color code")
